...
| `X402_PAY_TO`                                                                            | Destination address enforced for payouts      | Yes      | —                          |
...
Facilitator X402 rejects authorizations whose `pay_to` does not match `X402_PAY_TO`; callers remain responsible for constraining `asset` and `network`.
